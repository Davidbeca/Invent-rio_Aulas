(function () {
    var API_BASE = "/api/produtos";
    var STORAGE_KEY = "produtos.adminApiKey";
    ...
    function redirectFromQrValue(value) {
        ...
        window.location.href = "/produtos/codigo/" + encodeURIComponent(extractCodigo(value));
    }
    ...
    function extractCodigo(value) {
        ...
        var match = trimmed.match(/\/produtos\/codigo\/([^/?#]+)/i);
        if (match && match[1]) {
    ...
    function showStatusFromQuery() {
        ...
        if (status === "created") {
            ...
            showFeedback("Produto apagado com sucesso.", "success");
        }
    }

    function bindSearchBox() {
        var input = document.getElementById("produtoBusca");
        var btnBuscar = document.getElementById("btnProdutoBuscar");
        var btnLimpar = document.getElementById("btnProdutoLimpar");
    ...
    function filterProductsByName(query) {
        ...
        var rows = document.querySelectorAll("#produtos-lista tbody tr");
    ...
    window.deleteProduct = function (buttonElement) {
        ...
        var nome = row.querySelector("td:nth-child(2)").textContent || "este produto";
        ...
        fetch(API_BASE + "/" + id, {
            ...
            .then(function (response) {
                ...
                window.location.href = "/produtos?status=deleted";
            })
            .catch(function (error) {
